...
def _apply_feedforward(args, target):
...
def _servo_summary(servo_status):
    ...
    temps = []
    volts = []
    for data in servo_status.values():
        for key, out in (
            ("current_ma", currents),
            ("load_raw", loads),
            ("temperature_c", temps),
...
def _validate_trajectory(args, cache):
    ...
    for sample_idx in range(samples):
        ...
        command_target = _apply_feedforward(args, target)
    ...
    print("  z_feedforward_bias_m: %.6f" % float(args.z_feedforward_bias))
...
def _run_joint_trajectory_mode(args, cache, center):
    ...
    _, current_joints, _, _, _ = cache.snapshot()
    command_center = _apply_feedforward(args, center)
    ...
    if args.move_to_center_duration > 0.0:
        print(
            ...
            % (center_err, float(args.z_feedforward_bias))
        )
        ...
        if not center_ready:
            ...
            if args.strict_center or max_center_error > args.center_max_start_error:
                raise RuntimeError(
                    "%s\nThe path was not started. Do not enlarge joint limits to bypass this; "
                    "increase --move-to-center-duration or inspect shoulder tracking first." % message
                )
            print(
                "Warning: %s\n"
                "Continuing because max joint error is within --center-max-start-error=%.3frad."
                % (message, args.center_max_start_error),
                file=sys.stderr,
            )
        rospy.sleep(args.settle)
