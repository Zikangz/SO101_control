...
def _landmark_rows(rows, axis, direction):
    """Return samples near one geometric extreme of the nominal path."""
    column = "target_" + axis
    ...
    lower = min(values)
    upper = max(values)
    # Five percent of travel identifies the curved end region without mixing
    # it with the middle crossing of a figure eight.
    band = max(0.001, 0.05 * (upper - lower))
    threshold = lower + band if direction == "min" else upper - band
    result = []
    for row in rows:
        value = _float_or_none(row.get(column))
        if value is None:
...
def _mean_row_error(rows, measured_column, reference_column):
    values = []
    for row in rows:
        measured = _float_or_none(row.get(measured_column))
        reference = _float_or_none(row.get(reference_column))
        if measured is not None and reference is not None:
            values.append(measured - reference)
    if not values:
        return None
    return statistics.fmean(values), _rms(values), max(abs(value) for value in values)


def _analyze_path_landmarks(rows):
    """Quantify the visually important extrema of a Cartesian path."""
    if not rows or "target_x" not in rows[0] or "target_z" not in rows[0]:
        return
    labels = (
        ("left X edge", "x", "min"),
        ("right X edge", "x", "max"),
    ...
    print("path landmark tracking")
    print("  errors are feedback FK minus the stated target; values are millimetres")
    for label, axis, direction in labels:
        group = _landmark_rows(rows, axis, direction)
        nominal_z = _mean_row_error(group, "measured_z", "target_z")
        command_z = _mean_row_error(group, "measured_z", "command_target_z")
        nominal_x = _mean_row_error(group, "measured_x", "target_x")
        if nominal_z is None or nominal_x is None:
            continue
        command_text = "mean Z vs IK command=n/a"
        if command_z is not None:
            command_text = "mean Z vs IK command=%+.2f" % (command_z[0] * 1000.0)
        print(
            "  %-13s n=%-4d mean X=%+.2f mean Z=%+.2f rms Z=%.2f max Z=%.2f %s"
            % (
                label,
                len(group),
                nominal_x[0] * 1000.0,
...
def analyze_file(path, ignore_start=0.0, ignore_end=0.0):
    ...
    preferred = [
        ...
        "servo_min_voltage_v",
        "servo_shoulder_lift_current_ma",
        "servo_shoulder_lift_load_raw",
        "servo_shoulder_lift_voltage_v",
        "servo_shoulder_lift_temperature_c",
        "servo_shoulder_lift_velocity_raw",
        "servo_elbow_flex_current_ma",
        "servo_elbow_flex_load_raw",
        "servo_elbow_flex_voltage_v",
        "servo_elbow_flex_temperature_c",
        "servo_elbow_flex_velocity_raw",
        "servo_wrist_flex_current_ma",
        "servo_wrist_flex_load_raw",
        "servo_wrist_flex_voltage_v",
        "servo_wrist_flex_temperature_c",
        "servo_wrist_flex_velocity_raw",
    ]
