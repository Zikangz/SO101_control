...
from so101_ros1_bridge.backlash import DirectionalBacklashCompensator, load_backlash_profile
from so101_ros1_bridge.control import JointSafetyFilter
from so101_ros1_bridge.hardware import FeetechSO101Backend, MockSO101Backend
...
class SO101DriverNode:
    def __init__(self):
        ...
        self.disable_torque_on_relax = _bool_param("~disable_torque_on_relax", True)
        ...
        self.skip_duplicate_writes = _bool_param("~skip_duplicate_writes", True)
        self.configure_motors_on_connect = _bool_param("~configure_motors_on_connect", True)
        self.configuration_write_ack = _bool_param("~configuration_write_ack", True)
        ...
        self.feedback_position_assist_trajectory_only = _bool_param("~feedback_position_assist_trajectory_only", True)
        self.backlash_compensation_enabled = _bool_param("~backlash_compensation_enabled", False)
        self.backlash_compensation_profile = rospy.get_param("~backlash_compensation_profile", "")
        self.backlash_compensation_trajectory_only = _bool_param("~backlash_compensation_trajectory_only", True)
        self.backlash_compensation_joints = rospy.get_param(
            "~backlash_compensation_joints", ["shoulder_lift", "elbow_flex"]
        )
        self.backlash_compensation_max_abs_bias_rad = rospy.get_param(
            "~backlash_compensation_max_abs_bias_rad", ""
        )
        self.backlash_compensation_bias_slew_rad_s = rospy.get_param(
            "~backlash_compensation_bias_slew_rad_s", ""
        )
        self.backlash_compensation_velocity_threshold_rad_s = rospy.get_param(
            "~backlash_compensation_velocity_threshold_rad_s", ""
        )
        self.backlash_compensation_position_hysteresis_rad = rospy.get_param(
            "~backlash_compensation_position_hysteresis_rad", ""
        )
        self.backlash_compensation_limit_margin_rad = rospy.get_param(
            "~backlash_compensation_limit_margin_rad", ""
        )
        ...
        self.loop_timestamps = deque()
        self.feedback_timestamps = deque()
        ...
        self.backlash_compensator = self._make_backlash_compensator()
    ...
    def _make_backend(self):
        ...
        if self.backend_name == "feetech":
            return FeetechSO101Backend(
                ...
                configure_motors_on_connect=self.configure_motors_on_connect,
                configuration_write_ack=self.configuration_write_ack,
            )
        raise ValueError("Unsupported backend: %s" % self.backend_name)

    def _make_backlash_compensator(self):
        profile = {}
        if self.backlash_compensation_profile:
            profile = load_backlash_profile(self.backlash_compensation_profile)
            profile["source"] = self.backlash_compensation_profile
        elif self.backlash_compensation_enabled:
            raise ValueError("backlash_compensation_enabled requires backlash_compensation_profile")
        return DirectionalBacklashCompensator(
            self.joint_order,
            self.limits,
            self.active_joints,
            self.locked_joints,
            profile=profile,
            enabled=self.backlash_compensation_enabled,
            trajectory_only=self.backlash_compensation_trajectory_only,
            max_abs_bias_rad=self.backlash_compensation_max_abs_bias_rad,
            bias_slew_rad_s=self.backlash_compensation_bias_slew_rad_s,
            velocity_threshold_rad_s=self.backlash_compensation_velocity_threshold_rad_s,
            position_hysteresis_rad=self.backlash_compensation_position_hysteresis_rad,
            limit_margin_rad=self.backlash_compensation_limit_margin_rad,
            joints=self.backlash_compensation_joints,
        )
    ...
    def connect(self):
        ...
        self.backlash_compensator.reset(self.current_positions)
    ...
    def _reset_output_positions(self, positions):
        ...
        self.backlash_compensator.reset(self.output_positions)
    ...
    def _publish_status(self, stale=None):
        ...
        status = {
            ...
            "backlash_compensation": self.backlash_compensator.status(),
            "skip_duplicate_writes": self.skip_duplicate_writes,
            "configure_motors_on_connect": self.configure_motors_on_connect,
            "configuration_write_ack": self.configuration_write_ack,
            ...
            "effective_rates_hz": self._effective_rates(now),
            "transport": self.backend.write_statistics(),
    ...
    def _record_timestamp(self, timestamps, now):
        timestamps.append(float(now))
        cutoff = float(now) - self.rate_window_s
        while timestamps and timestamps[0] < cutoff:
            timestamps.popleft()

    def _effective_rates(self, _now):
        def rate(timestamps):
            if len(timestamps) < 2:
                return 0.0
            elapsed = max(1e-6, float(timestamps[-1]) - float(timestamps[0]))
            return float(len(timestamps) - 1) / elapsed

        return {
            "window_s": self.rate_window_s,
            "loop": rate(self.loop_timestamps),
            "feedback_read": rate(self.feedback_timestamps),
            "write": rate(self.write_timestamps),
        }
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            now = time.time()
            self._record_timestamp(self.loop_timestamps, now)
            ...
            with self.lock:
                ...
                try:
                    if feedback_period > 0.0 and now - last_feedback >= feedback_period:
                        ...
                        self._record_timestamp(self.feedback_timestamps, now)
                        self.last_backend_error = ""
                    stale = self._command_is_stale(now)
                    ...
                    if not self.estop and not self.relaxed:
                        ...
                        backlash_command = self.backlash_compensator.apply(
                            desired_command,
                            now=now,
                            trajectory_active=self.filter.trajectory_active(),
                        )
                        assisted_command = self._apply_feedback_position_assist(backlash_command, now, dt)
                        output_command = self._smooth_command(assisted_command)
                        self.backend.write_positions(output_command)
                        self._record_timestamp(self.write_timestamps, now)
                except Exception as exc:
