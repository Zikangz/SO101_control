...
class BaseSO101Backend:
    ...
    def write_statistics(self):
        """Return non-critical transport diagnostics for the ROS status topic."""
        return {}
...
class FeetechSO101Backend(BaseSO101Backend):
    ...
    def __init__(
        ...
        self.configure_motors_on_connect = bool(configure_motors_on_connect)
        self.configuration_write_ack = bool(configuration_write_ack)
        self.last_goal_raw = {}
        self.scs = None
        ...
        self.last_write_mode = "not_written"
        self.last_nonempty_write_mode = "not_written"
        self.last_sync_write_error = ""
        self.write_batches = 0
        self.sync_write_batches = 0
        self.individual_write_batches = 0
        self.register_write_batches = 0
    ...
    def connect(self):
        ...
        self._recover_serial_bus(delay_s=0.10)
        if self.configure_motors_on_connect:
            self._configure_motors()
            self._recover_serial_bus(delay_s=0.03)
    ...
    def _configure_motors(self):
        for joint in self.joint_order:
            self._write(joint, "Torque_Enable", 0, expect_status=self.configuration_write_ack)
            self._write(joint, "Return_Delay_Time", 0, expect_status=self.configuration_write_ack)
            self._write(joint, "Operating_Mode", 0, expect_status=self.configuration_write_ack)
            self._write(joint, "P_Coefficient", self._pid_value(joint, "p", 16), expect_status=self.configuration_write_ack)
            self._write(joint, "I_Coefficient", self._pid_value(joint, "i", 0), expect_status=self.configuration_write_ack)
            self._write(joint, "D_Coefficient", self._pid_value(joint, "d", 32), expect_status=self.configuration_write_ack)
            if self.servo_maximum_acceleration is not None:
                self._write(joint, "Maximum_Acceleration", self.servo_maximum_acceleration, expect_status=self.configuration_write_ack)
            self._write(joint, "Acceleration", self.servo_acceleration, expect_status=self.configuration_write_ack)
            if joint == "gripper":
                self._write(joint, "Max_Torque_Limit", 500, expect_status=self.configuration_write_ack)
                self._write(joint, "Protection_Current", 250, expect_status=self.configuration_write_ack)
                self._write(joint, "Overload_Torque", 25, expect_status=self.configuration_write_ack)
            self._write(joint, "Torque_Enable", 1, expect_status=self.configuration_write_ack)

    def _recover_serial_bus(self, delay_s=0.03):
        """Clear stale half-duplex bytes before retrying a servo transaction."""
    ...
    def _read(self, joint, data_name):
        ...
        for attempt in range(attempts):
            ...
            if attempt < attempts - 1:
                self._recover_serial_bus(delay_s=0.02 + 0.02 * attempt)
                continue
            break
        if comm != self.scs.COMM_SUCCESS:
    ...
    def _write(self, joint, data_name, value, expect_status=True):
        address, length = self.CONTROL_TABLE[data_name]
        motor_id = self.motor_ids[joint]
        raw = int(value)
        if data_name in ENCODE_SIGN_BIT:
        ...
        for attempt in range(attempts):
            with self.io_lock:
                if not expect_status:
                    if self.sdk_api == "bound":
                        comm = self.packet_handler.writeTxOnly(motor_id, address, length, data)
                    else:
                        comm = self.packet_handler.writeTxOnly(self.port_handler, motor_id, address, length, data)
                    err = 0
                elif self.sdk_api == "bound":
        ...
        if expect_status:
            ...
            self._recover_serial_bus(delay_s=0.003)
    ...
    def write_positions(self, positions):
        ...
        if not commanded:
            self.last_write_mode = "skipped_duplicates"
            return
        ...
        if self.sdk_api == "bound" and hasattr(self.packet_handler, "SyncWritePosEx"):
            ...
            with self.io_lock:
                try:
                    ...
                    if comm == self.scs.COMM_SUCCESS:
                        ...
                        self.last_write_mode = "sync_write_pos_ex"
                        self.last_nonempty_write_mode = self.last_write_mode
                        self.last_sync_write_error = ""
                        self.write_batches += 1
                        self.sync_write_batches += 1
                        self.last_write_duration_s = time.monotonic() - write_started
            ...
            if sync_error:
                ...
                self.last_sync_write_error = sync_error
        ...
        if self.sdk_api == "bound" and hasattr(self.packet_handler, "WritePosEx"):
            for joint, raw in commanded:
                ...
                self.last_goal_raw[joint] = raw
            self.last_write_mode = "individual_write_pos_ex"
            self.last_nonempty_write_mode = self.last_write_mode
            self.write_batches += 1
            self.individual_write_batches += 1
            self.last_write_duration_s = time.monotonic() - write_started
            self.last_write_time = time.time()
            return
        ...
        for joint, raw in commanded:
            ...
            self.last_goal_raw[joint] = raw
        self.last_write_mode = "individual_goal_position"
        self.last_nonempty_write_mode = self.last_write_mode
        self.write_batches += 1
        self.register_write_batches += 1
        self.last_write_duration_s = time.monotonic() - write_started
        self.last_write_time = time.time()

    def write_statistics(self):
        return {
            "last_write_mode": self.last_write_mode,
            "last_nonempty_write_mode": self.last_nonempty_write_mode,
            "last_sync_write_error": self.last_sync_write_error,
            "write_batches": self.write_batches,
            "sync_write_batches": self.sync_write_batches,
            "individual_write_batches": self.individual_write_batches,
            "register_write_batches": self.register_write_batches,
            "last_write_duration_s": self.last_write_duration_s,
